...
function processCases(cases: PaymentCase[], decisionOverrides: Record<string, Decision> = {}): Result[] {
  return cases.map((payment, index) => {
    ...
    const events: AuditEvent[] = [
      { id: `${payment.id}-1`, caseId: payment.id, timestamp: baseTime.toISOString(), kind: "diagnosis", title: "Failure diagnosed", detail: `${decision.diagnosis} · ${decision.confidence * 100}% confidence`, status: "info" },
      { id: `${payment.id}-2`, caseId: payment.id, timestamp: new Date(baseTime.getTime() + 45_000).toISOString(), kind: decision.action === "escalate_operator" ? "escalation" : "action", title: decision.action === "retry_payment" ? "Bounded retry simulated" : decision.action === "send_update_reminder" ? "Payment update reminder simulated" : "Escalated to operator", detail: decision.rationale, status: decision.requiresApproval ? "blocked" : "success" },
      { id: `${payment.id}-3`, caseId: payment.id, timestamp: new Date(baseTime.getTime() + 90_000).toISOString(), kind: recovered ? "verification" : "stop", title: recovered ? "Payment recovered" : outcome === "simulator_error" ? "Simulator error contained" : "Workflow stopped safely", detail: recovered ? `${formatInr(payment.amount)} verified in simulation` : decision.stopReason || "No further action permitted by policy", status: recovered ? "success" : "warning" },
    ];
...
export default function Home() {
  ...
  useEffect(() => {
    if (auditQuery.data && auditQuery.data.length === 0 && !localStorage.getItem("recoveriq-initial-audit-seeded")) {
      const initialEvents = processCases(initialCases).flatMap((item) => item.events.map((event) => ({ eventId: `initial-${event.id}`, caseId: event.caseId, kind: event.kind, title: event.title, detail: event.detail, status: event.status, eventTimestamp: event.timestamp })));
  ...
  const runBatch = async () => {
    ...
    const nextResults = processCases(nextCases, overrides);
      ...
      auditAppend.mutate(nextResults.flatMap((item) => item.events.map((event) => ({ eventId: `${Date.now()}-${event.id}`, caseId: event.caseId, kind: event.kind, title: event.title, detail: event.detail, status: event.status, eventTimestamp: event.timestamp }))), { onSuccess: () => trpcUtils.audit.list.invalidate() });
