...
function encodeWorkflowAudit(event: WorkflowEvent) {
...
function hydrateWorkflowAudit(event: { eventId: string; caseId: string; eventTimestamp: Date | string; kind: string; title: string; detail: string; status: string }): WorkflowEvent {
  try {
    const metadata = JSON.parse(event.detail) as Partial<WorkflowEvent> & { message?: string };
    return {
      id: event.eventId,
      caseId: event.caseId,
      timestamp: new Date(event.eventTimestamp).toISOString(),
      kind: event.kind as WorkflowEvent["kind"],
      title: event.title,
      detail: metadata.message || event.detail,
      status: event.status as WorkflowEvent["status"],
      state: metadata.state || "AUTOMATIC",
      previousState: metadata.previousState,
      nextAction: metadata.nextAction,
      policyDecision: metadata.policyDecision,
      policyRule: metadata.policyRule,
      actionResult: metadata.actionResult,
      recoveredAmount: metadata.recoveredAmount,
    };
  } catch {
    return { id: event.eventId, caseId: event.caseId, timestamp: new Date(event.eventTimestamp).toISOString(), kind: event.kind as WorkflowEvent["kind"], title: event.title, detail: event.detail, status: event.status as WorkflowEvent["status"], state: "AUTOMATIC" };
  }
}
...
export default function Home() {
  ...
  useEffect(() => {
    if (auditQuery.data && auditQuery.data.length === 0 && !localStorage.getItem("recoveriq-initial-audit-seeded")) {
      const initialEvents = processCases(initialCases).flatMap((item) => item.events.map((event) => ({ eventId: `initial-${event.id}`, caseId: event.caseId, kind: event.kind, title: event.title, detail: encodeWorkflowAudit(event), status: event.status, eventTimestamp: event.timestamp })));
  ...
  const replaySelected = () => {
    const replay = runRecoveryWorkflow(selected, classifyCase(selected));
    const replayRunId = `replay-${Date.now()}`;
    const replayEvents = replay.events.map((event) => ({ ...event, id: `${replayRunId}-${event.id}` }));
    ...
    auditAppend.mutate(replayEvents.map((event) => ({ eventId: event.id, caseId: event.caseId, kind: event.kind, title: event.title, detail: encodeWorkflowAudit(event), status: event.status, eventTimestamp: event.timestamp })), { onSuccess: () => trpcUtils.audit.list.invalidate() });
    setAuditTrail((previous) => {
      const nextTrail = [...previous, ...replayEvents];
  ...
  const exportCsv = () => {
    const csv = buildStakeholderCsv(results.map((item) => ({
      ...
      recovered: item.recovered, nextStep: item.nextStep, stopReason: item.stopReason || "", finalState: item.finalState, attempts: item.attempts, recoveredAmount: item.recoveredAmount,
    })), auditTrail);
  ...
  const runBatch = async () => {
    ...
    const nextResults = processCases(nextCases, overrides);
      ...
      auditAppend.mutate(nextResults.flatMap((item) => item.events.map((event) => ({ eventId: `${Date.now()}-${event.id}`, caseId: event.caseId, kind: event.kind, title: event.title, detail: encodeWorkflowAudit(event), status: event.status, eventTimestamp: event.timestamp }))), { onSuccess: () => trpcUtils.audit.list.invalidate() });
      setAuditTrail((previous) => {
